...
**All 522 catalogued Excel functions are recognized**, but recognition is not the same as full Excel-compatible execution. The function catalog now exposes availability explicitly: real implementations, real-but-not-fully-verified implementations, environment-bound functions, and intentionally unavailable fixed-error stubs. The unavailable set is limited to host-service / connection functions that require infrastructure Formulon does not embed (for example PY, WEBSERVICE, STOCKHISTORY, IMAGE, RTD, TRANSLATE, DETECTLANGUAGE, COPILOT, and the CUBE* OLAP family). `make function-status` reports the current split.
