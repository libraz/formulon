...
- **Strict oracle, not aspirational compatibility.** The runtime default is `win-365-ja_JP`, and profile-specific oracle suites pin observed Excel behavior. The primary checked-in oracle is Mac Excel 365 (ja-JP). The Windows `win-365-ja_JP` target is currently `wanted`: its historical Office 2019 / unknown-version files are retained as `reference-only` and are not evidence of a verified Microsoft 365 capture or active CTest coverage. Outputs are checked for bit-level parity against golden data regenerated from the real product; every accepted divergence (transcendental ulp drift, volatile-function snapshots, Excel quirks where Formulon deliberately keeps a saner answer) is recorded case-by-case in [`tests/divergence.yaml`](tests/divergence.yaml) with a reason and the last verified Excel build.
...
**97 oracle categories** are defined and regenerated from Mac Excel 365 ja-JP. Windows Excel 365 ja-JP is an external, currently pending target; its historical reference files are excluded from active coverage. Current local verification:
