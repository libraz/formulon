...
Beyond formula results, **pivot tables and print areas / pagination** have a dedicated **workbook oracle track**: a print/pagination engine (`src/print/`), a pivot evaluator/layout, and declarative pivot/print oracle cases are in place, verified by the `formulon_workbook_oracle_tests` harness. Because reliable PivotTable automation needs Windows Excel COM, this track's primary is `win-365-ja_JP`; the oracle harness and cases are committed, but golden capture against Excel is pending a Windows Excel run.
