...
// A blank copied out of a worksheet reference becomes a cell of the derived
// array, so COUNTA sees it. `TOROW` over the same source already promotes
// through `materialise_vector`, which makes it the reference answer.
TEST(BuiltinsWrap, ReferenceBlanksAreCountedAsArrayCells) {
  Workbook wb = Workbook::create();
  Sheet& sheet = wb.sheet(0);
  // A1:D1 is intentionally left blank.
  EvalState state;
  const EvalContext ctx = test::mac_context(wb, sheet, state);
  Arena parse_arena;
  Arena eval_arena;

  const Value baseline = EvalUnder("=COUNTA(TOROW(A1:D1))", &parse_arena, &eval_arena, ctx);
  ASSERT_TRUE(baseline.is_number()) << "kind=" << static_cast<int>(baseline.kind());
  EXPECT_DOUBLE_EQ(baseline.as_number(), 4.0);

  parse_arena.reset();
  eval_arena.reset();
  const Value by_row = EvalUnder("=COUNTA(WRAPROWS(A1:D1, 2))", &parse_arena, &eval_arena, ctx);
  ASSERT_TRUE(by_row.is_number()) << "kind=" << static_cast<int>(by_row.kind());
  EXPECT_DOUBLE_EQ(by_row.as_number(), baseline.as_number());

  parse_arena.reset();
  eval_arena.reset();
  const Value by_col = EvalUnder("=COUNTA(WRAPCOLS(A1:D1, 2))", &parse_arena, &eval_arena, ctx);
  ASSERT_TRUE(by_col.is_number()) << "kind=" << static_cast<int>(by_col.kind());
  EXPECT_DOUBLE_EQ(by_col.as_number(), baseline.as_number());
}

// A pad cell is generated by WRAP itself rather than copied from the source,
// so an explicitly blank pad projects to zero at the grid boundary and still
// counts as an occupied array cell — matching EXPAND's pad rule.
TEST(BuiltinsWrap, BlankPadProjectsAtGridBoundary) {
  Workbook wb = Workbook::create();
  Sheet& sheet = wb.sheet(0);
  ...
  const Value v = EvalUnder("=WRAPROWS(A1:C1, 2, )", &parse_arena, &eval_arena, ctx);
  ASSERT_TRUE(v.is_array());
  ASSERT_EQ(v.as_array_rows(), 2U);
  ASSERT_EQ(v.as_array_cols(), 2U);
  ...
  parse_arena.reset();
  eval_arena.reset();
  const Value counta = EvalUnder("=COUNTA(WRAPROWS(A1:C1, 2, ))", &parse_arena, &eval_arena, ctx);
  ASSERT_TRUE(counta.is_number());
  EXPECT_DOUBLE_EQ(counta.as_number(), 4.0);
}
