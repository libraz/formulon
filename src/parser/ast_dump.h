...
// contract for M2.6's 2,000-case parser corpus, so the output must remain
// stable across platforms and compiler versions: it is locale-free, uses
// neither `<iostream>` nor `printf`-style floating-point formatting, and
// trims trailing zeros from non-integer numerics.
//
...
///
/// See the table in `backup/plans/02-calc-engine.md` §2.3 (and the M2.3a
/// implementation notes) for the per-kind format. Output is deterministic
